...
export default function BloodInventoryPage() {
  ...
  const load = async () => {
    ...
    try {
      const [invRes, hospRes, alertRes] = await Promise.all([
        api.get('/api/blood-inventory'),
        api.get('/api/hospitals'),
        api.get('/api/blood-inventory/low-stock/alerts'),
      ])
  ...
  const saveEdit = async (invId) => {
    try {
      await api.patch(`/api/blood-inventory/${invId}`, { bloodStock: editStock })
  ...
  const addInventory = async () => {
    ...
    try {
      await api.post('/api/blood-inventory', { hospitalId: newHospital, bloodStock: newStock })
        ...
        <div className="card p-5 mb-4 animate-fade-in">
          ...
          <div className="mb-4">
            ...
            <select className="input-field" value={newHospital} onChange={e => setNewHospital(e.target.value)}>
              ...
              {(() => {
                const available = hospitals.filter(h => !inventories.find(i => i.hospitalId?._id === h._id));
                if (available.length === 0 && hospitals.length > 0) {
                  return <option disabled>All hospitals already have an inventory (Edit below)</option>;
                }
